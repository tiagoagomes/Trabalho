 ...
 */
function initializeLinter(inputRules, environments = [], globals = [], workingDirectory, linterId = 'default') {
    (0, shared_1.debug)(`Initializing linter "${linterId}" with ${inputRules.map(rule => rule.key)}`);
    linters[linterId] = new wrapper_1.LinterWrapper({ inputRules, environments, globals, workingDirectory });
}
/**
