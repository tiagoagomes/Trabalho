...
function hasSonarRuntimeOption(ruleModule, ruleId) {
    ...
    if (Array.isArray(schema)) {
        return schema.some(option => !!option.enum && option.enum.includes(exports.SONAR_RUNTIME));
    }
    if (schema?.type === 'array' && Array.isArray(schema.items)) {
        return schema.items.some(option => option.type === 'string' && option.enum?.includes(exports.SONAR_RUNTIME));
    }
    return false;
}
